...
if env.str("AWS_PARAMETER_PATH", None) is None:
    ...
    # database connection from db/creentoials viz. DATABASE_URL
    if 'db/credentials' in param_dict:
        DATABASE_URL = aws.aws_secret_to_psql_url(param_dict['db/credentials'], "us-east-1", "webstats")
        
# TODO DEBUG WIP RECONSIDER INVOLKING AS PARAMETER TO  app.config.from_pyfile(config_filename)
